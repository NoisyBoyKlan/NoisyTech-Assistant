...
function loginUser(uuid, clientip, callback){
	if(uuid==0){
		...
		if(typeof database[uuid]=="undefined"){
			return registerUser(clientip, callback), database[uuid]["userVars"];
		} else {
			return callback({"uuid": uuid}), database[uuid]["userVars"];
		}
	}
}
...
module.exports = function(req, res, uuid, language, message, callback){
	...
	loginUser(uuid, clientip, function(data, userVars){
		var userCredentials = data;
		messageUser(userCredentials.uuid, clientip, language, message, function(){
			require("../core/core.js")(userCredentials, language, message, callback, userVars);
		});
	});
}